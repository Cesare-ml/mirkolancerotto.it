...
var VERSION3 = new Version("20.3.4");

// sito-artista/src/app/services/generated-data-url.ts
var previewSessionVersion = Date.now().toString(36);
function generatedDataUrl(fileName) {
  return `assets/data/${fileName}?v=${previewSessionVersion}`;
}

// sito-artista/src/app/services/site.service.ts
var SiteService = class _SiteService {
  ...
  constructor(http) {
    this.http = http;
    this.siteConfig$ = this.http.get(generatedDataUrl("site.json")).pipe(shareReplay(1));
    this.pagesConfig$ = this.http.get(generatedDataUrl("pages.json")).pipe(shareReplay(1));
    this.series$ = this.http.get(generatedDataUrl("series.json")).pipe(shareReplay(1));
  }
  getSiteConfig() {
  ...
  localize(text, language) {
    ...
    return text[language] || text.it || "";
  }
  static \u0275fac = function SiteService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _SiteService)(\u0275\u0275inject(HttpClient));
  };
...
// sito-artista/src/app/services/language.service.ts
var LanguageService = class _LanguageService {
  ...
  getRouteLanguage(route) {
    return route.data["language"] ?? "it";
  }
  getRoutePageKey(route) {
    return route.data["pageKey"] ?? "home";
  }
  localize(text, language) {
    if (!text) {
      return "";
  ...
  buildLocalizedPath(language, pageKey, params) {
    const basePath = `/${language}`;
    const segment = this.pageSegments[language][pageKey];
    if (pageKey === "home") {
      return `${basePath}/`;
    }
    if (pageKey === "artwork-detail" && params?.slug) {
      return `${basePath}/${segment}/${params.slug}`;
    }
    if (pageKey === "auction-detail" && params?.slug) {
      return `${basePath}/${segment}/${params.slug}`;
    }
...
// sito-artista/src/app/services/seo.service.ts
var SeoService = class _SeoService {
  document;
  languageService;
  constructor(document2, languageService) {
    this.document = document2;
    this.languageService = languageService;
  }
  applyLocalizedAlternates(domain, route, params, defaultLanguage = "it") {
    const pageKey = this.languageService.getRoutePageKey(route);
    const currentLanguage = this.languageService.getRouteLanguage(route);
    this.setLinkTag("canonical", `${domain}${this.languageService.buildLocalizedPath(currentLanguage, pageKey, params)}`.replace(/\/$/, ""));
    this.setLinkTag("alternate-it", `${domain}${this.languageService.buildLocalizedPath("it", pageKey, params)}`.replace(/\/$/, ""), "alternate", "it");
    this.setLinkTag("alternate-en", `${domain}${this.languageService.buildLocalizedPath("en", pageKey, params)}`.replace(/\/$/, ""), "alternate", "en");
    this.setLinkTag("alternate-default", `${domain}${this.languageService.buildLocalizedPath(defaultLanguage, pageKey, params)}`.replace(/\/$/, ""), "alternate", "x-default");
  }
  applyPageMetadata(options) {
    this.document.title = options.title;
    this.setDocumentLanguage(options.language || "it");
    this.setMetaTag("description", options.description);
    this.setMetaTag("robots", options.robots || "index,follow");
    this.setMetaTag("og:title", options.title, "property");
    this.setMetaTag("og:description", options.description, "property");
    this.setMetaTag("og:type", options.type || "website", "property");
    this.setMetaTag("og:url", options.url, "property");
    this.setMetaTag("og:site_name", options.siteName || "", "property");
    this.setMetaTag("og:locale", this.mapLocale(options.language || "it"), "property");
    this.setMetaTag("twitter:card", options.twitterCard || "summary_large_image");
    this.setMetaTag("twitter:title", options.title);
    this.setMetaTag("twitter:description", options.description);
    if (options.image) {
      this.setMetaTag("og:image", options.image, "property");
      this.setMetaTag("twitter:image", options.image);
    }
  }
  applyFavicon(href) {
    if (!href) {
    ...
    let linkElement = this.document.head.querySelector('link[rel="icon"]');
    if (!linkElement) {
  ...
  applyStructuredData(key, payload) {
    const selector = `script[data-seo-key="${key}"]`;
    let scriptElement = this.document.head.querySelector(selector);
    if (!scriptElement) {
      scriptElement = this.document.createElement("script");
      scriptElement.type = "application/ld+json";
      scriptElement.setAttribute("data-seo-key", key);
  ...
  clearStructuredData(key) {
    const selector = `script[data-seo-key="${key}"]`;
    const scriptElement = this.document.head.querySelector(selector);
    scriptElement?.remove();
  }
  setLinkTag(key, href, rel = "canonical", hreflang) {
    const selector = `link[data-seo-key="${key}"]`;
    let linkElement = this.document.head.querySelector(selector);
    if (!linkElement) {
      linkElement = this.document.createElement("link");
      linkElement.setAttribute("data-seo-key", key);
      this.document.head.appendChild(linkElement);
    }
  ...
  setMetaTag(name, content, attribute = "name") {
    ...
    let metaElement = this.document.head.querySelector(selector);
    if (!metaElement) {
  ...
  mapLocale(language) {
    return language === "en" ? "en_US" : "it_IT";
  }
  static \u0275fac = function SeoService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _SeoService)(\u0275\u0275inject(DOCUMENT), \u0275\u0275inject(LanguageService));
  };
...
@angular/router/fesm2022/router.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-5ZWV2562.js.map
